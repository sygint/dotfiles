...
# Provides convenient functions to access fleet-config.nix throughout your flake
{
...
let
  ...
  getHost =
    hostName:
    networkConfig.hosts.${hostName} or (throw "Host '${hostName}' not found in fleet-config.nix");
  ...
  getWolMac =
    ...
    let
      host = getHost hostName;
    in
    if hasWol hostName then
      host.wol.mac
    else
      throw "Host '${hostName}' does not have Wake-on-LAN enabled";
  ...
  getDeployConfig =
    ...
    let
      host = getHost hostName;
    in
    if isDeployable hostName then
      host.deploy
    else
      throw "Host '${hostName}' is not configured for deployment";

  # Get list of all deployable hosts
  # Usage: getDeployableHosts => [ "cortex" ]
  getDeployableHosts = builtins.filter (name: (getHost name).deploy.enabled or false) (
    attrNames networkConfig.hosts
  );

  # Get list of all hosts with WoL enabled
  # Usage: getWolHosts => [ "cortex" ]
  getWolHosts = builtins.filter (name: (getHost name).wol.enabled or false) (
    attrNames networkConfig.hosts
  );
