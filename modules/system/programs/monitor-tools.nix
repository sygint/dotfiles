...
let
  ...
  cfg = config.modules.programs.monitor-tools;
  ...
  scriptSrc = if builtins.pathExists scriptSrcFromSelf then scriptSrcFromSelf else if builtins.pathExists scriptSrcLocal then scriptSrcLocal else null;
...
in
{
  options.modules.programs.monitor-tools.enable = mkEnableOption "Monitor tools and scripts globally available";

  config = mkIf cfg.enable {
    environment.systemPackages = [
      # Read the script from the repo root's scripts/desktop directory; correct
      # relative path from this module file (modules/system/programs/) is
      ...
      (if monitorSetupPackage != null then monitorSetupPackage else (pkgs.writeShellScriptBin "monitor-setup" (let
        scriptText = if scriptExists then builtins.readFile scriptSrc else ''#!/usr/bin/env bash
